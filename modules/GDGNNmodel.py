    ...
    def forward(self, batch_data):
       ...
       # print("size of batch data",batch_data.x)
        ...
        if not self.args.eval and self.args.prior_type in ['Dir2',
            ...
            # print("condits")
            # print("recon_word",recon_word.shape)
            # print("KL1",KL1.shape)
...
def myscattersum(src, index, size=None):
    ...
    # return: B, C1,C2
    # print("src from gdgnn",src)
    # print("B*len from gdgnn",B*len)
    if src is None:
    ...
    ids = torch.where(flag > 0)[0]  # the coordinates of the last point of each sample sequence
    nids = ids[1:] - ids[:-1]  # LEN-1
    nids = torch.cat([ids[[0]] + 1, nids], dim=0)  # LEN  how many points are there per sample sequence
    ids = ids[:-1] + 1  # LEN-1
...
if __name__ == '__main__':
    ...
    print("x from gdgnn", x)
    print("x_batch from gdgnn", x_batch)
