...
def init_config():
    ...
    if args.model_type in ['GDGNNMODEL']:
        model_str = '_%s_ns%d_ench%d_ni%d_nw%d_ngram%d_temp%.2f-%.2f' % \
                    (args.model_type, args.num_samp, args.enc_nh, args.ni, args.nw, args.nwindow, args.INITIAL_TEMP,
                     args.MIN_TEMP)
...
def main(args):
    ...
    test_loader = DataLoader(test_data, batch_size=args.batch_size, shuffle=False,drop_last=True, follow_batch=['x', 'edge_id', 'y'])
    val_loader = DataLoader(val_data, batch_size=args.batch_size, shuffle=False,drop_last=True ,follow_batch=['x', 'edge_id', 'y'])
    ...
    #print("word_vecc from new place",word_vec)
    #print(word_vec.shape)
    #print("t3")
    word_vec = torch.from_numpy(word_vec).float()
    #word_vec=word_vec[0:len(word_vec)]
    #print("reached upto args.model_type")
    ...
    #print("trainloader_length",len(train_loader))
    ...
    with torch.no_grad():
        test(model, test_loader, 'TEST')
        torch.cuda.empty_cache()
        if 'News20' in args.dataset:
            refpath = todatapath('News20')
            data = pd.read_csv(refpath + '/overall%s.csv' % stop_str, header=0, dtype={'label': int, 'train': int})

        else:
            data = pd.read_csv(path + '/overall%s.csv' % stop_str, header=0,
                               dtype={'label': int, 'train': int})  # data = data[data['train'] > 0]
        common_texts = [text for text in data['content'].values]
        beta = model.get_beta().detach().cpu().numpy()
        eval_topic(beta, [vocab.id2word(i) for i in range(args.vocab_size)], common_texts=common_texts)
        #if args.dataset in LABELED_DATASETS:
