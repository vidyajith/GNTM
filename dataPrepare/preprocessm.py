...
def process_dataset(dataset='AG', STOPWORD=True):
    if STOPWORD:
        load_stopwords()
    if dataset == 'BNC':
        path = BNC_ADDR + '/raw/'
        ...
        data1 = open("C:/Users/vidhy/PycharmProjects/GNTM2/tagmynews.txt", encoding="utf8").readlines()
        #print("data1",data1[0:])

        print("length_data1",len(data1))
        print("data1[19998]",data1[19998])

        print("data1[6]",data1[6])
        print("shape_data1", np.shape(data1))
        ...
        for i in range(N):
            ...
            if label not in ldct:
                ...
                id = len(ldct)
                # print("id",id)
                ldct[label] = id  # 1,2,3,4,5,6....
                #print("ldct[label",ldct[label])
            l = ldct[label]
            # print("l",l)
            # print("ldct",ldct)
        ...
        print("DATA",data)
        data.to_csv("data_used2.csv")
        N = len(data)
        print("N",N)
        SN = 9000
        #SN=1000
        # random.choice(a,size=None,replace=True,p=None)
        ids = np.random.choice(range(N), size=SN, replace=True, p=None)
        print("ids",ids)
        N2 = len(ids)
        print("N2", N2)
        ...
        print("data.head..............")
        print(data.head())
        print("length(data[train])", len(data["train"]))
        ...
        # print(data1[0])
        print("shape_data1", np.shape(data1))
        N = int(len(data1))  # initial....5001
        # N = int(len(data1) / 4)
        # print("length of data divided by 4",N)
        ldct = {}
        values = []
        for i in range(N - 1):  # 0 to 5001
            content = data1[i]  # data1[8*1]
            if i <= 1600:
                content = content[31:]
            else:
                content = content[37:]
...
def clean_vocab(path, STOPWORD=False, freq_threshold=5):
    ...
    for line in data['content'].values:
        #line= re.sub('[a-zA-Z"/''0-9//.:'']', '',data["content"][line])
        sents = line.strip()
    ...
    if STOPWORD:
        ...
        print("entered non stop word condition")
...
def eos_unk_pad_emd(nw=300):
    EOS_UNK_PAD={}
    with open(GLOVE_ADDR + 'glove.B.{}d.txt'.format(nw), 'rb') as fd:
        for l in fd:
            line = l.decode().split()
            word = line[0]
            if word in ['pad', 'unk', 'eos']:
